...
#include "visco_plastic_compressible.h"
...
namespace aspect
{
  namespace MaterialModel
  {
      ...
      for (unsigned int i=0; i < in.temperature.size(); ++i)
        {
          ...
          for (unsigned int j=0; j < volume_fractions.size(); ++j)
            {
              ...
              const double pressure_factor = std::exp(compressibilities[j] * (pressure - this->get_surface_pressure()));
          ...
          for (unsigned int j=0; j < volume_fractions.size(); ++j)
            compressibility += volume_fractions[j] * compressibilities[j];
          ...
          for (unsigned int c=0; c<in.composition[i].size(); ++c)
            {
              if (c == this->introspection().compositional_index_for_name("continent"))
                {
                  if (this->get_geometry_model().depth(in.position[i]) < 150000.0)
                    out.reaction_terms[i][c] = 1.0 - in.composition[i][c];
                  else
    ...
    is_compressible () const
    {
      ...
      for (unsigned int j=0; j<compressibilities.size(); ++j)
        if (compressibilities[j]>0)
          {
    ...
    template <int dim>
    double ViscoPlasticCompressible<dim>::
    ...
    template <int dim>
    void
    ViscoPlasticCompressible<dim>::declare_parameters (ParameterHandler &prm)
    {
      prm.enter_subsection("Material model");
      {
        prm.enter_subsection ("Visco Plastic");
        {
          ...
          prm.declare_entry ("Compressibilities", "4e-12",
                             Patterns::List(Patterns::Double(0)),
                             "List of compressibilities for background material and compositional fields. "
                             "Units: $1/Pa$.");
    ...
    template <int dim>
    void
    ViscoPlasticCompressible<dim>::parse_parameters (ParameterHandler &prm)
    {
      ...
      prm.enter_subsection("Material model");
      {
        prm.enter_subsection ("Visco Plastic");
        {
          ...
          thermal_expansivities = Utilities::possibly_extend_from_1_to_N (Utilities::string_to_double(Utilities::split_string_list(prm.get("Thermal expansivities"))),
                                                                          n_fields,
                                                                          "Thermal expansivities");
          compressibilities = Utilities::possibly_extend_from_1_to_N (Utilities::string_to_double(Utilities::split_string_list(prm.get("Compressibilities"))),
                                                                      n_fields,
                                                                      "Compressibilities");
    ...
    template <int dim>
    void
    ViscoPlasticCompressible<dim>::create_additional_named_outputs (MaterialModel::MaterialModelOutputs<dim> &out) const
    {
...
namespace aspect
{
  namespace MaterialModel
  {
    ASPECT_REGISTER_MATERIAL_MODEL(ViscoPlasticCompressible,
                                   "visco plastic compressible",
