...
#include <aspect/material_model/fractionation.h>
  ...
  namespace MaterialModel
  {
             ...
             MaterialModel::MaterialModelOutputs<dim> &out) const
    {
      for (unsigned int i=0; i < in.position.size(); ++i)
        {
          const double ref_temperature = this->get_adiabatic_conditions().temperature(in.position[i]);
          const double delta_temp = in.temperature[i]-ref_temperature;
          const double temperature_dependence = (reference_T > 0
                                                 ?
                                                 std::max(std::min(std::exp(-thermal_viscosity_exponent*delta_temp/ref_temperature),
    ...
    template <int dim>
    void
    Fractionation<dim>::declare_parameters (ParameterHandler &prm)
    {
      prm.enter_subsection("Material model");
      {
        prm.enter_subsection("Fractionation model");
        {
          ...
          prm.declare_entry ("Density differential for compositional field 1", "0",
                             ...
                             "to make the density depend on these fields. In this fractionation material "
    ...
    template <int dim>
    void
    Fractionation<dim>::parse_parameters (ParameterHandler &prm)
    {
      prm.enter_subsection("Material model");
      {
        prm.enter_subsection("Fractionation model");
        {
          ...
          if (thermal_viscosity_exponent!=0.0 && reference_T == 0.0)
            AssertThrow(false, ExcMessage("Error: Material model fractionation with Thermal viscosity exponent can not have reference_T=0."));
        }
        prm.leave_subsection();
      }
      prm.leave_subsection();
...
namespace aspect
{
  namespace MaterialModel
  {
    ASPECT_REGISTER_MATERIAL_MODEL(Fractionation,
                                   "fractionation",
                                   ...
                                   "from a section ``Material model/Fractionation model'' in the input file, see "
                                   "Section~\\ref{parameters:Material_20model/Fractionation_20model}."
                                   "\n\n"
                                   ...
                                   "\n\n"
                                   "\\note{Despite its name, this material model is not exactly ``fractionation'', "
