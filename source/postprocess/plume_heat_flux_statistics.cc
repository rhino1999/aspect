...
#include <aspect/postprocess/plume_heat_flux_statistics.h>
...
namespace aspect
{
  namespace Postprocess
  {
    ...
    PlumeHeatFluxStatistics<dim>::execute (TableHandler &statistics)
    {
            ...
            if (cell->at_boundary(f))
              {
                ...
                for (unsigned int q=0; q<fe_face_values.n_quadrature_points; ++q)
                  {
                    const double specific_heat = out.specific_heat[q];
                    const double deltaT = in.temperature[q] - this->get_adiabatic_surface_temperature();
                    const double density = out.densities[q];
                    const double normal_velocity = in.velocity[q] * fe_face_values.normal_vector(q);

                    local_normal_flux
                    +=
                      -specific_heat *
                      (in.temperature[q] *density *
                       normal_velocity) *
      ...
      for (std::map<types::boundary_id, double>::const_iterator
        ...
        {
          const std::string name = "Inward plume heat flux through boundary with indicator "
      ...
      return std::pair<std::string, std::string> ("Plume heat fluxes through boundary parts:",
                                                  screen_text.str());
    }
  }
}
...
namespace aspect
{
  namespace Postprocess
  {
    ASPECT_REGISTER_POSTPROCESSOR(PlumeHeatFluxStatistics,
                                  "plume heat flux statistics",
