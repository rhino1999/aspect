          ...
          if (in.strain_rate.size() > 0)
            {
              ...
              if (std::sqrt(strain_rate_effective) <= std::numeric_limits<double>::min())
              ...
              else
                {
                  ...
                  double effective_cohesion = cohesion;
                  Point<dim> center;
                  center[0] = 20000.0;
//
//                  if ((in.position[i]-center).norm() < 1000)
//                    effective_cohesion = cohesion / 10.0;
//                  effective_cohesion *= (1 - seed_cohesion_reduction *
//                      std::exp(-1.0*(in.position[i]-center).norm_square() / (seed_extent * seed_extent)));

                  double strength = ( (dim==3)
                                            ?
                                            ( 6.0 * effective_cohesion * cos_phi + 2.0 * pressure * sin_phi) * strength_inv_part
                                            :
                                            effective_cohesion * cos_phi + pressure * sin_phi );
                  ...
                  const double effective_plastic_viscosity = 1.0 / ( ( 1.0 / ( eta_plastic + minimum_viscosity ) ) + ( 1.0 / maximum_viscosity ) );
...
//                                                         :
//                                                         1.0);
//                  const double effective_thermal_viscosity = 1e21 * ;

                  Assert(dealii::numbers::is_finite(effective_plastic_viscosity), ExcMessage ("Error: Viscosity is not finite."));

                  out.viscosities[i] = effective_plastic_viscosity;
                  ...
                  if (derivatives != NULL)
                    {
                      const double averaging_factor = effective_plastic_viscosity * effective_plastic_viscosity * std::pow(eta_plastic + minimum_viscosity,-2);
    ...
    template <int dim>
    void
    DruckerPragerSeed<dim>::declare_parameters (ParameterHandler &prm)
    {
      prm.enter_subsection("Material model");
      {
        prm.enter_subsection("Drucker Prager seed");
        {
    ...
    template <int dim>
    void
    DruckerPragerSeed<dim>::parse_parameters (ParameterHandler &prm)
    {
      prm.enter_subsection("Material model");
      {
        prm.enter_subsection("Drucker Prager seed");
...
namespace aspect
{
  namespace MaterialModel
  {
    ASPECT_REGISTER_MATERIAL_MODEL(DruckerPragerSeed,
                                   "drucker prager seed",
                                   ...
                                   "from a section ``Material model/Drucker Prager seed'' in the input file, see "
                                   ...
                                   "\n\n"
                                   "The viscosity is computed according to the Drucker Prager seed frictional "
                                   ...
                                   "yield surface. "
                                   "In 2D the Drucker Prager seed yield surface is the same "
